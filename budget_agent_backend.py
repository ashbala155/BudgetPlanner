...
DEFAULT_BUDGET_INPUTS = {
    ...
    "fixed_costs": 2200.0,
    "variable_costs": 1100.0,
    "debt_payments": 300.0,
    "savings_goal": 6000.0,
    "priority": "Balanced plan",
}
...
def load_user_budget_inputs(username: str) -> dict[str, Any]:
    if is_supabase_configured():
        client = _get_supabase_client()
        user_id = _get_supabase_user_id(client, username)
        response = (
            client.table("budget_user_profiles")
            .select("monthly_income,fixed_costs,variable_costs,debt_payments,savings_goal,priority")
            .eq("user_id", user_id)
            .limit(1)
            .execute()
        )
        if response.data:
            return {**DEFAULT_BUDGET_INPUTS, **response.data[0]}

    profile_path = _profile_path_for_user(username)
    profile = _read_json_object(profile_path)
    if profile:
        return {**DEFAULT_BUDGET_INPUTS, **profile}

    history_df = load_budget_history(username)
    if not history_df.empty:
        latest = history_df.sort_values(by="saved_at").iloc[-1]
        return {
            "monthly_income": float(latest.get("income", DEFAULT_BUDGET_INPUTS["monthly_income"])),
            "fixed_costs": float(latest.get("fixed_costs", DEFAULT_BUDGET_INPUTS["fixed_costs"])),
            "variable_costs": float(latest.get("variable_costs", DEFAULT_BUDGET_INPUTS["variable_costs"])),
            "debt_payments": float(latest.get("debt_payments", DEFAULT_BUDGET_INPUTS["debt_payments"])),
            "savings_goal": float(latest.get("savings_goal", DEFAULT_BUDGET_INPUTS["savings_goal"])),
            "priority": str(latest.get("priority", DEFAULT_BUDGET_INPUTS["priority"])),
        }
    return DEFAULT_BUDGET_INPUTS.copy()


def save_user_budget_inputs(
    username: str,
    ...
    if is_supabase_configured():
        client = _get_supabase_client()
        user_id = _get_supabase_user_id(client, username)
        client.table("budget_user_profiles").upsert(
            {
                "user_id": user_id,
                **payload,
            }
...
def _profile_path_for_user(username: str) -> Path:
    normalized_username = normalize_username(username)
    user_dir = _ensure_user_dir(normalized_username)
    return user_dir / "budget_profile.json"
