...
def get_auth_users() -> dict[str, str]:
    ...
    auth_section: Any = st.secrets.get("auth", {})

    if hasattr(auth_section, "get"):
        nested_users = auth_section.get("users", {})
        if hasattr(nested_users, "items"):
            for username, value in nested_users.items():
                if hasattr(value, "get"):
                    password = str(value.get("password", "")).strip()
                else:
                    password = str(value).strip()
                if password:
                    auth_users[str(username).strip()] = password

        single_username = str(auth_section.get("username", "")).strip()
        single_password = str(auth_section.get("password", "")).strip()
        if single_username and single_password:
            auth_users[single_username] = single_password

    env_username = os.getenv("APP_USERNAME", "").strip()
    env_password = os.getenv("APP_PASSWORD", "").strip()
    if env_username and env_password:
        auth_users[env_username] = env_password
...
def login_gate() -> None:
    ...
    if not auth_required:
        ...
        st.session_state.current_user = "guest"
        st.info("Login secrets are not configured, so the app is running in demo mode.")
        return
    ...
    st.markdown(
        ...
        <div class="hero-card">
            ...
            <h1 style="margin-bottom:0.2rem; color:#0f172a;">Sign in to Budget Planning AI Agent</h1>
            <p style="font-size:1.0rem; color:#334155; max-width:720px;">
                Use the username and password stored in your Streamlit secrets or environment variables.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with st.form("login_form"):
        ...
        if submitted:
            ...
            valid_user = bool(stored_password)
            valid_password = valid_user and hmac.compare_digest(password, stored_password)
            if valid_password:
                st.session_state.authenticated = True
                st.session_state.current_user = username
                st.rerun()
            else:
                st.error("Those credentials did not match the configured login.")
    st.stop()
